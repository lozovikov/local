...
<?php			$i = 1;
					foreach ($MODULE_OUTPUT["abit_specs"] as $spec) { 
						if (!empty($spec["code"])) {
							echo '<a href="/'.($spec["form"] == 'заочная' ? 'izop/abiturientam/' : ($spec["type"] == 'magistracy' ? 'magistrantu/' : 'entrant/')).($spec["form"] == 'вечерняя' ? 'list-vpo-ev' : ($spec["form"] == 'спо' ? 'list-spo' : ( $spec["type"] == 'magistracy' ? 'list' : 'list' )/*'list'*/ ) ).'/?type='.$spec["type_id"].'" title="Список поступающих на специальность  &quot;'.$spec["name"].'&quot;">'.$spec["code"];
							...
							if (!empty($spec["name"]) && $spec["name"])
								$to_show[] = '<a href="/'.($spec["form"] == 'заочная' ? 'izop/abiturientam/' : ($spec["type"] == 'magistracy' ? 'magistrantu/' : 'entrant/')).($spec["form"] == 'вечерняя' ? 'list-vpo-ev' : ($spec["form"] == 'спо' ? 'list-spo' : ( $spec["type"] == 'magistracy' ? 'list' : 'list' )/*'list'*/ ) ).'/?type='.$spec["type_id"].'" title="Список поступающих на специальность  &quot;'.$spec["name"].'&quot;">'.$spec["name"].'</a>';
