...
<?php					/*$messages = $MODULE_OUTPUT["messages"];
			...
			<form action="<?=$EE["unqueried_uri"]?>" method="post">
				...
				<textarea name="<?=$NODE_ID?>[save][text]"  cols="60" rows="15" class="wysiwyg" ><?php echo ($MODULE_OUTPUT["text"]); ?></textarea>
...
<?php		}
			...
			if ($flag2) {
				...
				<p class="actions">
					[&nbsp;<a class="<?=$class?>" href="<?=$EE["unqueried_uri"]?>?node=<?=$NODE_ID?>&amp;action=edit#form&amp;mode=modal&amp;item_id=<?=$MODULE_OUTPUT["item_id"]?>" onmouseover="document.getElementById('node<?=$NODE_ID?>').style.backgroundColor = '#95c27c'" onmouseout="document.getElementById('node<?=$NODE_ID?>').style.backgroundColor = ''">
					Редактировать текст</a>&nbsp;]</p>
			</div>
<?php		}
		}
		break;
