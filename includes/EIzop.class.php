...
// date: 2014-07-24
{
	...
	function EIzop($global_params, $params, $module_id, $node_id, $module_uri)
	{
		...
		switch($this->mode){
			...
			break;
			case "izop_old":
				$parts2 = explode("/", rtrim($Engine->unqueried_uri, "/"));
				if(isset($parts[1])) $branch = $parts[1];
				else $branch = $parts2[count($parts2)-1]; 				 
				$this->output["mode"] = $this->output["scripts_mode"] = "izop_old"; 
				$spec_id = isset($_GET['spec_id']) ? $_GET['spec_id'] : false;
				if(isset($_GET['spec'])) $spec_id = $_GET['spec'];
				if(isset($_GET['form'])) $this->output["form"] = (int)$_GET['form'];
	...
	function requests_info(){
		...
		$DB->SetTable("nsau_spec_abit", "sa"); 
	    $DB->AddTable($this->db_prefix."types", "t");
		$DB->AddField("sa.type_id");
	    $DB->AddCondFF("sa.type_id", "=", "t.id"); 
		$DB->AddExp("count(*) as req_count");
		$DB->AddCondFS("sa.form", "=", "заочная");
		$DB->AddJoin("nsau_qualifications q.id", "sa.qualification", "nsau_spec_abit");
		$DB->AddCondFS("q.old_type", "!=", "secondary");
    $DB->AddAltFS("q.old_type", "=", "higher");
      $DB->AddAltFS("q.old_type", "=", "bachelor");
      $DB->AppendAlts();
		//$DB->AddCondFS("qualification", "!=", "51");
		$DB->AddGrouping("sa.type_id");
	    //echo $DB->SelectQuery(); 
		$res = $DB->Select();
		while($row = $DB->FetchAssoc($res)) {
	...
	function list_entrants($type_id = 0){
        ...
        $this->output["exams"] = array();  
	            ...
	            $DB->SetTable("nsau_exams", "e");
	            $DB->AddTable("nsau_spec_exams", "se");
	            $DB->AddTable("nsau_spec_type", "st"); 
	            $DB->AddJoin("nsau_qualifications q.id", "se.qual_id", "nsau_spec_exams"); 
			...
			$DB->AddTable("nsau_spec_abit", "a");
			$DB->AddJoin("nsau_abit_types t.position", "a.position");
			//$DB->AddFields(array("a.position", "a.documents", "a.ege"));
			$DB->AddExp("p.*");
			$DB->AddExp("a.*");
      $DB->AddExp("t.*");
      $DB->AddField("ac.title", "acceptance"); 
      //$DB->AddExp("*");
			$DB->AddCondFS("a.form", "=", "заочная");
			//$DB->AddCondFS("a.qualification", "!=", "51");
		    //$DB->AddCondFO("a.qualification");
			$DB->AddJoin("nsau_qualifications q.id", "a.qualification", "nsau_spec_abit"); 
      $DB->AddJoin("nsau_acceptances ac.acceptance", "a.acceptance", "nsau_spec_abit");
			$DB->AddCondFS("q.old_type", "!=", "secondary"); 
      $DB->AddCondFS("q.old_type", "!=", "magistracy");
      $DB->AddAltFS("q.old_type", "=", "higher");
      $DB->AddAltFS("q.old_type", "=", "bachelor");
      $DB->AppendAlts(); 
