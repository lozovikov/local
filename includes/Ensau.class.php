...
// date: 2014-08-19
{
	...
	function search_people($search_query = false) {
		...
		foreach ($search_query_parts as $ind=>$part) {
			...
			if (strtoupper($part[0]) != $part[0])
				...
				//$DB->exec("SELECT * FROM nsau_people");
					$res = $DB->Select();				
	...
	*/
	...
	if (isset($_GET["gr"])) {
		...
		WHERE x.id=".htmlspecialchars($_GET["gr"]);
		...
		WHERE x.group_id=".htmlspecialchars($_GET["gr"]);
		...
		if ($_GET["gr"]) {
			...
			$DB->AddCondFS("group_id", "=", htmlspecialchars($_GET["gr"]));
			$res = $DB->Select();
			if ($row = $DB->FetchAssoc($res))
    ...
    function people($module_uri)
    {
        ...
        if(!empty($module_uri))
        {
            ...
            if(($new_spec['type'] == "higher" || $new_spec['type'] == "bachelor")  && $new_spec['form'] == 'заочная')
            $DB->AddCondFS("m.name", "=", "EIzop"); 
            else 
            $DB->AddCondFS("m.name", "=", "EAbit");
            $DB->AddExp("n.*");
            //$DB->AddCondFS("module_id", "=", $this->module_id);
            if($new_spec['type'] == "magistracy" && $new_spec['form'] == 'очная') $DB->AddCondFS("n.params", "REGEXP", " *list;[0-9];magistracy *$");
            if($new_spec['type'] == "magistracy" && $new_spec['form'] == 'заочная') $DB->AddCondFS("n.params", "REGEXP", " *list;[0-9];magistracy-ex *$");
            if(($new_spec['type'] == "higher" || $new_spec['type'] == "bachelor")  && $new_spec['form'] == 'очная') $DB->AddCondFS("n.params", "REGEXP", " *list;[0-9] *$");
            if(($new_spec['type'] == "higher" || $new_spec['type'] == "bachelor")  && $new_spec['form'] == 'вечерняя') $DB->AddCondFS("n.params", "REGEXP", " *list;[0-9];vpo-ev *$");
            if(($new_spec['type'] == "higher" || $new_spec['type'] == "bachelor")  && $new_spec['form'] == 'заочная') $DB->AddCondFS("n.params", "REGEXP", " *list *$");
            if($new_spec['type'] == "secondary" && $new_spec['form'] == 'очная') $DB->AddCondFS("n.params", "REGEXP", " *list;[0-9];spo *$");
            if($new_spec['type'] == "secondary" && $new_spec['form'] == 'заочная') $DB->AddCondFS("n.params", "REGEXP", " *list;[0-9];spo-ex *$");
            //echo $DB->SelectQuery(); 
            $res2 = $DB->Select(1);
	...
	function timetable_make()
	{
		...
		if (isset($fac_id)) {
			$sql = "SELECT x.id as id, x.name as name, y.name as kaf FROM nsau_subjects x LEFT JOIN nsau_departments y ON y.id = x.department_id WHERE y.faculty_id=".$fac_id." ORDER BY kaf, name";
			$res = $DB->Exec($sql);
		}
		else {
