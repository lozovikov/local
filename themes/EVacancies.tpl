	...
	switch ($MODULE_DATA["output"]["mode"])
	{
      ...
      <a href="/about/department/practice_placement/">Отдел практик и трудоустройства</a>
			...
			foreach ($MODULE_DATA["output"]["vacancies_list"] as $vac) {
				...
				echo "<tr".$bgcolor."><td><a href=\"".$vac["id"]."/\">".$vac["vacancy"]."</a><br><small><a href=\"?{$NODE_ID}[filter][company]=".urlencode($vac["company"])."\" title=\"Показать все вакансии этой организации\" style=\"color: #000000; \">".$vac["company"]."</a>".$manage."</td><td>".$vac["salary"]."</small></td><td>".$vac["district"]."</td><td>".$vac["lodging"]."</td><td>".$vac["pub_date"]."</td></tr>";
				$i++;
			}
			echo "</table>";
