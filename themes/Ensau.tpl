					...
					foreach ($weeks as $wnum => $wname) {
						...
						if ($MODULE_OUTPUT["mode"] == "timetable_make") { ?>
							<tr<?=$trstyle?> id="tr_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_double<? } ?>">
								...
								<td valign="top">c <input type="text" class="comm" style="width: 100px;" readonly="readonly" id="comment_from_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>" name="comment_from_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>" value="<? echo $curpair["comment_from"]=='0000-00-00' ? '' : implode(".",array_reverse(explode("-",$curpair["comment_from"])))?>" /> по <input type="text" style="width: 100px;" id="comment_to_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>" class="comm" name="comment_to_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>" readonly="readonly" value="<? echo $curpair["comment_to"]=='0000-00-00' ? '' : implode(".",array_reverse(explode("-",$curpair["comment_to"])))?>" /></td>

								<? if ($MODULE_OUTPUT["mode"]=="timetable_make") { ?><td valign="center"><select size="5" multiple="multiple" style="max-width: 70px;" name="copy_<?=$wnum?>_<?=$day?>_<?=$pnum?>[]"><? foreach ($MODULE_OUTPUT["potok"] as $key => $val) { ?><option value="<?=$key?>"><?=$val?></option><? } ?></select><!--<input type="checkbox" name="copy_<?=$wnum?>_<?=$day?>_<?=$pnum?>" onclick="if(this.checked) { if (!confirm('Внимание! Отметив эту галочку, Вы замените расписание этой пары для всех групп потока. Вы уверены?')) return false; }" />--></td><? } ?>
							</tr>
						<? } else {  /* timetable_show */
							?>
							<tr<?=$trstyle?>>
