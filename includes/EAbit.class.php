...
// date: 2014-07-24
{
    ...
    */   
			...
			case "abitfile": {
				...
				if (isset($_FILES['abit'])) { 
					...
					if ($fname[count($fname)-1] != "csv") {
						...
						if($_POST["type"] == "spo" || $_POST["type"] == "spo-ex") $fieldNum = 25; 
						$toobad = 0;
						foreach ($parts as $ind => $part) {
              //print_r($part);//exit;
							if ($part) {
								$string = explode(";", $part);//print_r($string); 
								if (count($string) > 1 && count($string) != 17 && count($string) != $fieldNum && !$toobad) {
									$this->output["messages"]["bad"][] = "Неверное количество полей: ".count($string)." вместо ".$fieldNum;
									$toobad = 1;
								} elseif (count($string) > 1 && !$toobad) {
									$fieldNames = array("f1" => 1, "f2" => 2, "f3" => 3/*, "fd" => 8*/, "f5" => 9, "f6" => 10, "fout" => 13, "ftook" => 14,"fqual" => 23);
                    ...
                    if ($ind2==13 || $ind2==14)
									...
									if($_POST["type"] == "spo" || $_POST["type"] == "spo-ex") $fieldNames[] = "fclass";
									$DB->SetTable("abit_temp");
			...
			case "edit_spec": {
		        ...
		        if(isset($_POST["mode"]) && $_POST["mode"] == "add") {
			      ...
			      if(!$row) {
			        ...
			        if(!empty($_POST["commerce_ext"])) $DB->AddValue("commerce_ext", $_POST["commerce_ext"]);
			        if(!empty($_POST["classes"])) $DB->AddValue("classes", $_POST["classes"]);
			        if(!empty($_POST["has_ext"])) $DB->AddValue("has_ext", $_POST["has_ext"]);
	...
	function People($isSpo = 0, $isEvening = 0, $isExt = 0, $isMagistracy = 0)
	{
		...
		if (isset($_GET["type"]))
			$sql = "select p.*, a.*, t.*, coalesce(q.name, a.qual_name) as qual_name from nsau_people p, nsau_spec_abit a left join nsau_abit_types t on t.position=a.position left join ".$this->db_prefix."qualifications q on q.id=a.qualification where a.out=0 and a.took=0 and a.form='".($isEvening ? 'вечерняя' : ($isSpo ? ($isExt ? 'заочная' : 'очная') : ($isMagistracy ? ($isExt ? 'заочная' : 'очная') : 'очная')))."' and a.people_id=p.id and a.type_id=".$_GET["type"]." and a.speciality_id ".($isSpo ? " REGEXP '[[:alnum:]]{2}\.02\.[[:alnum:]]{2}' " : " NOT REGEXP '[[:alnum:]]{2}\.02\.[[:alnum:]]{2}' ")." ".($isMagistracy ? " and q.old_type = 'magistracy' " : "")." order by a.position, a.status1, a.dogovor desc, replace(a.ege, ',', '.') + 0.0 desc, a.status2 desc, p.last_name";
		else
			$sql = "select * from nsau_people p, nsau_spec_abit a left join nsau_abit_types t on t.position=a.position where a.out=0 and a.took=0 and a.form='".($isEvening ? 'вечерняя' : ($isSpo ? ($isExt ? 'заочная' : 'очная') : ($isMagistracy ? ($isExt ? 'заочная' : 'очная') : 'очная')))."' and a.people_id=p.id and a.speciality_id ".($isSpo ? " REGEXP '[[:alnum:]]{2}\.02\.[[:alnum:]]{2}' " : " NOT REGEXP '[[:alnum:]]{2}\.02\.[[:alnum:]]{2}' ")." group by p.last_name, p.name, p.patronymic order by a.position, a.status1, a.dogovor desc, (replace(a.ege, ',', '.') + 0.0) desc, a.status2 desc, p.last_name";
...
/*		if (isset($_GET["type"])) {
	...
	function GetSpecsInfo() {
      ...
      if(isset($_GET["qual"])) { 
		...
		$rows = array();
		$sum = array("has_ext" => false, "budget" => 0, "budget_ext" => 0, "commerce" => 0, "commerce_ext" => 0, "purpose" => 0);
		while($row = $DB->FetchObject())
		{	
	...
	{	
		...
		$DB->AddField($this->db_prefix  ."types.code");
		$DB->AddField($this->db_prefix  ."types.type");
		if($request_mode != "spo-ex" && $request_mode != "magistracy-ex")
			$DB->AddField($this->db_prefix  ."types.budget");
		else 
		...
		if($request_mode == "magistracy-ex") $DB->AddCondFS("params", "REGEXP", " *list;[0-9];magistracy-ex *$");
		if($request_mode == "magistracy") $DB->AddCondFS("params", "REGEXP", " *list;[0-9];magistracy *$");
		if($request_mode == "vpo-ev") $DB->AddCondFS("params", "REGEXP", " *list;[0-9];vpo-ev *$");
		if($request_mode == "spo") $DB->AddCondFS("params", "REGEXP", " *list;[0-9];spo *$");
    if($request_mode == "spo-ex") $DB->AddCondFS("params", "REGEXP", " *list;[0-9];spo-ex *$");
		//echo $DB->SelectQuery(); 
		$res = $DB->Select(1);
