...
document.write('<table id="fc" style="position:absolute;z-index: 10000; border-collapse:collapse;background:#FFFFFF;border:1px solid #303030;display:none;-moz-user-select:none;-khtml-user-select:none;user-select:none;" cellpadding=2>');
...
for(var kk=1;kk<=6;kk++) {
	...
	for(var tt=1;tt<=7;tt++) {
		num=7 * (kk-1) - (-tt);
		document.body.write('<td id="v' + num + '" style="width:18px;height:18px">&nbsp;</td>');
	}
	document.write('</tr>');
}
...
var updobj;
var user_callback;
function lcs(ielem, callback) {
	user_callback = callback ? callback : null;
	updobj=ielem;
	getObj('fc').style.left=Left(ielem)+'px';
	getObj('fc').style.top=(Top(ielem)+ielem.offsetHeight-0)+'px';
...
function prepcalendar ( hd, cm, cy )
{
	...
	for(var d=1;d<=42;d++)
	{
		...
		if ( ( d >= (cd -(-1) ) ) && ( d<=cd-(-marr[cm]) ) )
		{
			...
			calvalarr[d]=addnull(cy, cm-(-1), (d-cd < 10 ? '0'+(d-cd) : d-cd) );
		}
		else
		{
