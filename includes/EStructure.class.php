		...
		if(!isset($this->output["ajax_mode"]) || $this->output["ajax_mode"] == "normal") {
			if(isset($_POST['modul'])) {
				if($_POST['modul'] == "structure") {
					...
					$json = array();
					$this->output["mode"] = "ajax_autocomplete_folder";
					$DB->SetTable("engine_folders", "e_fold");
					...
					$DB->AddCondFS("e_fold.title", "LIKE", "".iconv("utf-8", "Windows-1251", $_REQUEST['data']["q"])."%");
					$DB->AddField("e_fold.title","id");
					$DB->AddField("e_fold.title","pid");
					$DB->AddField("e_fold.title", "title");
					//$DB->AddField("dep.id", "did");
					//$DB->AddField("dep.name", "dname");
					
					//$DB->AddOrder("dep.name");//echo $DB->SelectQuery(); 
					$res = $DB->Select();
					//$privs = $Engine->GetPrivileges($this->module_id, "files.attach", null, $Auth->usergroup_id);print_r($privs);
					
				
					$count = 0; 
					while($row = $DB->FetchAssoc($res)) {
						if(!$Engine->OperationAllowed($this->module_id, "folder.subFolders.handle", $row["pid"], $Auth->usergroup_id) ) continue;
						if($row["title"]) $row["id"] = $row["pid"] . " > ".$row["title"];
						$json[] = $row;
