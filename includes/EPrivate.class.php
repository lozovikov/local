...
// date: 2014-07-30
{
    ...
    $_SESSION['private_node_id'] = $this->node_id;
    ...
    if(isset($_SESSION["old_user"])) $this->output["old_user"] = $_SESSION["old_user"];
		...
		if (ALLOW_AUTH)
		{
            ...
            elseif(isset($_GET[$this->node_id]['loginas'])) {
            $DB->SetTable("auth_users");
						$DB->AddCondFS("id", "=", $_GET[$this->node_id]['loginas']);
						$res = $DB->Select(1);
            if ($row = $DB->FetchObject($res)) {
              $old_user = array("key" => md5(time().date("Ymd H:i:s").rand())); 
              foreach($_SESSION as $key => $value) {
                if(preg_match('/^user(group)?_/', $key)) { $old_user[$key] = $value; }
              }
              $_SESSION["user_id"] = $row->id;
              $_SESSION["username"] = $row->username;
              $_SESSION["user_displayed_name"] = $row->displayed_name;
              $_SESSION["usergroup_id"] = $row->usergroup_id;
              //$_SESSION["usergroup_name"] = $this->usergroup_name;
              //$_SESSION["usergroup_comment"] = $row->usergroup_comment;
              $Auth->SetSessionLifetime(); 
              $Auth->GetAFMRequestData();
              //$_SESSION["usergroup_session_lifetime"] = $this->usergroup_session_lifetime;
              $_SESSION["old_user"] = $old_user;
            }
						//$Engine->LogAction($this->module_id, "user", $parts_uri[3], "delete");
						CF::Redirect($Engine->FolderURIbyID(1)); 
            //CF::redirect($Engine->engine_uri);
            }
            
            elseif(isset($_GET[$this->node_id]["relogin"], $_GET[$this->node_id]["key"], $_SESSION["old_user"])) {//echo $Engine->engine_uri;exit;
              if($_GET[$this->node_id]["relogin"] == $_SESSION["old_user"]["user_id"] && $_GET[$this->node_id]["key"] == $_SESSION["old_user"]["key"]) {
                $Auth->ClearAFMRequestData();
                unset($_SESSION["old_user"]["key"]);  
                foreach($_SESSION["old_user"] as $key => $value) {
                  $_SESSION[$key] = $value;
                }
              ...
              CF::redirect($Engine->engine_uri);
					...
					{
						$this->output["status"] = false;
					}//echo $_GET[$this->node_id]["relogin"];print_r($_GET);print_r($_SESSION);
					break;
