...
elseif (( $MODULE_DATA["output"]["mode"]!="files_search" && $MODULE_DATA["output"]["mode"]!="link_to_file" && $MODULE_DATA["output"]["mode"]!="get_file" && $MODULE_DATA["output"]["mode"]!="files_list" && $MODULE_DATA["output"]["mode"]!="attach_file" && $MODULE_DATA["output"]["mode"]!="download") && $MODULE_DATA["output"]["mode"]!="els" && !isset($_SESSION["user_id"]) || ($MODULE_DATA["output"]["mode"]=="els" && substr($Auth->ip, 0, 7) != '192.168'   && !$MODULE_DATA["output"]["allow_download"])) {
...
<?php 										} ?>
												Вид: <?=(isset($attach["spec_view"]) ? $attach["spec_view"] : "Вид не указан")?><br />
						...
						<div class="center-notes">
							<div class="cn">
								<div class="form-notes">
									...
									<div><input name="upload_author" type="text" value=""/></div>
									<div>
...
<?php 										} ?>
												...
												<?php if (isset($dates["attach"][$id])) { ?><br />Добавлена: <?=$dates["attach"][$id] ?> <?php } ?>
