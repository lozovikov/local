...
class EGerman
{
    ...
    var $node_id;
    var $captcha_settings;//настройки капчи
    var $display_variant;
    var $form_data;
    var $config;
    
    function EGerman ($global_params, $params, $module_id, $node_id = NULL, $module_uri = NULL)
	{
        global $Engine, $DB;
        ...
        switch ($this->mode) {
            case "user_request"://форма заявки
			...
			case "requests_list": // список заявок
				global $DB;
				$DB->SetTable("german_requests");
				$res = $DB->Select();
			...
			if (isset($POST_DATA)) {
				...
				switch ($this->mode) {
					...
					case "user_request":
						...
						if (isset($_POST[$this->node_id]["code"])&&($_POST[$this->node_id]["code"] == $_SESSION["code"])&& isset($_SESSION["code"])
                            && CF::IsNonEmptyStr($_POST[$this->node_id]["code"])&& isset($POST_DATA["program"], $POST_DATA["name"], $POST_DATA["email"])
                            && CF::IsNonEmptyStr($POST_DATA["program"])&& CF::IsNonEmptyStr($POST_DATA["name"])&& CF::IsNonEmptyStr($POST_DATA["email"])) {
