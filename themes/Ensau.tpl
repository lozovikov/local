...
<?php								} ?>
<?php 
      //foreach($MODULE_OUTPUT["files"] as $file_type) {
      foreach($type_sp["files"] as $spec_file) {
        ?><a href="/file/<?php echo $spec_file['file_id'];?>"><?php echo $spec_file['file_type']; ?></a>&nbsp;<?php 
      }
      //}
      ?>
		</fieldset>
...
<?php			$i = 1;
					foreach ($MODULE_OUTPUT["abit_specs"] as $spec) { 
						if (!empty($spec["code"])) {
							echo '<a href="'.$spec['abit_list_base_href'].$spec["type_id"].'" title="Список поступающих на специальность  &quot;'.$spec["name"].'&quot;">'.$spec["code"];
							...
							if (!empty($spec["name"]) && $spec["name"])
								$to_show[] = '<a href="'.$spec['abit_list_base_href'].$spec["type_id"].'" title="Список поступающих на специальность  &quot;'.$spec["name"].'&quot;">'.$spec["name"].'</a>';
				...
				<div class="wide" id="newuser">
					<div class="form-notes">
						<dl>
							...
							<dd><input name="username" type="text" value="<?=($MODULE_DATA["output"]["cur_user"] && isset($man["username"]) ? $man["username"] : "")?>" />
            <?php if($Engine->OperationAllowed(0, "userGroup.users.loginAs", $user_group, $Auth->usergroup_id)){?><a href="<?=$EE["engine_uri"];?>?<?=$_SESSION['private_node_id'];?>[loginas]=<?=(isset($MODULE_DATA["output"]["cur_user"]) ? $MODULE_DATA["output"]["cur_user"] : 0)?>">Зайти как пользователь</a><?php } ?></dd>
					...
					<?php if($MODULE_DATA["output"]['allow_peopledit']) { ?><div class="wide">
						<dl id="usergroup" >
...
<?php
	...
	case "prof_choice": { ?>
		...
		<tr><td><input type="checkbox" name="ege_math_on" id="ege_math_on" /> Математика:</td><td><input type="text" size="3" name="ege_math" id="ege_math"></td></tr>
...
<?php		} ?>
							...
							<dd id="group_qualification">
								<select name="qualif">
<?php		$qualif = array(68=>"Магистратура", 65=>"Специалитет", 62=>"Бакалавриат", 51=>"Среднее специальное");
...
<?php		$form_edu = array(1=>"Очная", 2=>"Заочная", 3=>"Очно-заочная");
...
$form_edu = array(  1=>"Очная", 2=>"Заочная", 3=>"Очно-заочная" );
				foreach($form_edu as $form_code => $form_edu) {
...
<?php				} else { ?>
				...
				if($group["form_education"]==NULL) {
				$form_edu = array( 1=>"Очная", 2=>"Заочная" , 3=>"Очно-заочная" , NULL=>" ");
				foreach($form_edu as $form_code => $form_edu) {
					if($form_code == $group["form_education"]) {?>
									<option value="<?=$form_code?>" selected="selected"><?=$form_edu?></option>
<?php				} else { ?>
...
<?php 				}
	...
	<tr><td>
	<table style="width: 500px; background-color: #eee;" cellspacing="3">
		<tr>
			<td style="width: 250px;">Форма обучения</td>
			<td><select onchange="selgroups();" id="edform"><option></option><option value="1">Очная</option><br /><option value="2">Заочная</option><option value="3">Очно-заочная</option></select></td>
		</tr>
		<tr>
			<td style="width: 250px;">Факультет</td>
		...
		<tr>
			<td style="width: 250px;">Группа</td>
			<td><table><tr><td width="40%"><?php foreach ($MODULE_OUTPUT["groups"] as $gr) { ?><span onclick="location.href='?gr=<?=$gr["id"]?>';" style="clear:both; display:none;" class="groups gr_<?=$gr["form_education"]?>_<?=$gr["id_faculty"]?>_<?=$gr["year"]?>"><input type="radio" value="<?=$gr["id"]?>" id="<?=$gr["id"]?>" name="groups"> <?=$gr["name"]?></span><? } ?></td></tr></table></td>
		</tr>
		</table></td>
	</tr></table>
        <?php
	...
	$types = array("Лекция", "Практика", "Лаб. работа");
	...
	if (isset($_GET["gr"])) {
		...
		<? if ($MODULE_OUTPUT["mode"]=="timetable_show") { ?>
			...
			<h2>Направление подготовки: <strong><?=$MODULE_OUTPUT["group"]["specialities_code"]?> - <?=$MODULE_OUTPUT["group"]["spec"]?></strong></h2>
			<p>Группа <strong><?=$MODULE_OUTPUT["group"]["name"]?></strong>, <?=$MODULE_OUTPUT["group"]["year"]?> курс<br /> <strong>Подгруппа: <? foreach ($subgroups as $snum => $sg) { if ($_GET["subgr"]!=$snum) { ?><a href="?gr=<?=$_GET["gr"]?>&subgr=<?=$snum?>"><?=$sg?></a> <? } else echo $sg." "; } ?></strong></p><p><strong><a href="#" onclick="window.print();" id="aprint">Распечатать</a></strong></p>
		<h2 style="text-align: center;<? if (!$MODULE_OUTPUT["dates"]["actual_from"] && !$MODULE_OUTPUT["dates"]["actual_to"]) { ?> display: none;<? } ?>"><strong>Расписание занятий на период с <?=implode(".",array_reverse(explode("-",$MODULE_OUTPUT["dates"]["actual_from"])))?> по <?=implode(".",array_reverse(explode("-",$MODULE_OUTPUT["dates"]["actual_to"])))?></strong></h2>
		<? } else { ?>
			<br /><br /><h2 style="text-align: center;"><strong>Расписание занятий на период с <input type="text" id="actual_from" readonly="readonly" name="actual_from" value="<?=implode(".",array_reverse(explode("-",$MODULE_OUTPUT["dates"]["actual_from"])))?>" style="100px;" /> по <input type="text" id="actual_to" readonly="readonly" name="actual_to" value="<?=implode(".",array_reverse(explode("-",$MODULE_OUTPUT["dates"]["actual_to"])))?>" style="100px;" /></strong></h2>
		<? } ?>
		<? /* for jquery */ ?>
		<select style="display:none;" id="subjlist"><optgroup><option selected="selected"></option>
									<? $kaf = ""; foreach ($MODULE_OUTPUT["subjects"] as $k => $v) { if ($kaf != $v["kaf"]) { $kaf = $v["kaf"]; ?>
										</optgroup><option disabled="disabled"></option><optgroup label="<?=$kaf?>">
									<? } ?>
			<option value="<?=$k?>"<? if($curpair && $curpair["subject_id"] == $k) { ?> selected="selected"<? } ?>><?=$v["name"]?></option><? } ?></optgroup>
		</select>
		...
		<select style="display:none;" id="audlist"><option></option><? foreach ($MODULE_OUTPUT["auds"] as $k => $v) { ?><option value="<?=$k?>"><?=$v["label"]?> - <?=$v["aud_name"]?></option><? } ?>
			...
			foreach ($days as $day => $dname) {
				if (/*$MODULE_OUTPUT["mode"] == "timetable_make" || count($MODULE_OUTPUT[$wname][$day])*/ 1 /*выводим все пары*/) {
				?><br /><h2><?=$dname?></h2>
				<table width="100%" style="text-align: center;">
					<tr>
						...
						<? if ($MODULE_OUTPUT["mode"]=="timetable_make") { ?><th>Весь поток</th><? } ?>
					</tr>
					<? $rownum = 0;
					foreach ($pairs as $pnum => $pr) {
					foreach ($weeks as $wnum => $wname) {
						$pairzz = array(0 => "", 1 => ""); //для разных подгрупп
						...
						if ($MODULE_OUTPUT["mode"] == "timetable_make") { ?>
							<tr<?=$trstyle?> id="tr_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_double<? } ?>">
								...
								<td valign="top"><select style="max-width: 300px;" class="subjs" def="<?=$curpair["subject_id"]?>" name="subj_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>" id="subj_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>"></select><br /><input type="text" style="width: 100px; font-style: italic; color: #aaa;" value="начните ввод..." onfocus="if (this.value=='начните ввод...') this.value='';" onkeyup="select_filter('subj_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>', this.value, 'subjlist');" /></td>
								...
								<td valign="top"><span<? if (!$curpair || !$curpair["type"]) $dispnone = ' display: none;'; else $dispnone = ''; ?> style="max-width: 230px;<?=$dispnone?>" id="subgr_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>"><? foreach ($subgroups as $snum => $subgr) { ?><input type="radio" name="subgr_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>" value="<?=$snum?>"<? if($curpair && $curpair["subgroup"] == $snum) { ?> checked="checked"<? } ?> /> <?=$subgr?><? } ?><? if (!$is_double && !$pairzz[1]) { ?><p id="addouble_<?=$wnum?>_<?=$day?>_<?=$pnum?>"><a cursor="pointer" onclick="show_doublepair('<?=$wnum?>','<?=$day?>_<?=$pnum?>');"><small>Добавить пару для второй подгруппы</small></a></p><? } ?></span></td>
								...
								<td valign="top">c <input type="text" class="comm" style="width: 100px;" readonly="readonly" id="comment_from_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>" name="comment_from_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>" value="<?=implode(".",array_reverse(explode("-",$curpair["comment_from"])))?>" /> по <input type="text" style="width: 100px;" id="comment_to_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>" class="comm" name="comment_to_<?=$wnum?>_<?=$day?>_<?=$pnum?><? if ($is_double) { ?>_d<? } ?>" readonly="readonly" value="<?=implode(".",array_reverse(explode("-",$curpair["comment_to"])))?>" /></td>

								<? if ($MODULE_OUTPUT["mode"]=="timetable_make") { ?><td valign="center"><input type="checkbox" name="copy_<?=$wnum?>_<?=$day?>_<?=$pnum?>" onclick="if(this.checked) { if (!confirm('Внимание! Отметив эту галочку, Вы замените расписание этой пары для всех групп потока. Вы уверены?')) return false; }" /></td><? } ?>
							</tr>
						<? } else {  /* timetable_show */
							?>
							<tr<?=$trstyle?>>
								<? if(!isset($arr[$day][$pnum])) { ?><td valign="middle" rowspan="<?=$rowspan_count?>"><?=$pr?></td><? } ?>
								<td><? echo ($wname=="even") ? 'ЧЕТ' : 'НЕЧЕТ'; ?></td>
								<td><?=$curpair["subj"]?><? if ($curpair["comment_from"] && $curpair["comment_from"]!='00.00.0000') { ?><br /><span style="font-size: 12px;">с <?=$curpair["comment_from"]?> по <?=$curpair["comment_to"]?></span><? } ?></td>
								<td><?=$types[$curpair["type"]]?></td>
								<td><? echo $curpair["subgroup"] ? $subgroups[$curpair["subgroup"]] : ""; ?></td>
								<td><? foreach ($curpair["auds"] as $aud) { ?><?=$aud["build"]." - ".$aud["name"]?><br /><? } ?></td>
								<td><? foreach ($curpair["teachs"] as $teach) { ?><a href="/people/<?=$teach["people_id"]?>/" target="_blank"><?=$teach["p2"]." ".substr($teach["p0"],0,1).". ".substr($teach["p1"],0,1)?>.</a><br /><? } ?></td>
							</tr>
						<? } ?>
					<? $arr[$day][$pnum] = 1; } } } ?>
				</table><?
	...
	?></div><!-- Закрываем div основного содержимого расписания --><?
	if ($MODULE_OUTPUT["mode"]=="timetable_make") { ?>
...
<?php                   foreach($dep["subj_list"] as $subj) { ?>
            <li id='subj_item_<?=$subj["subj_id"]?>'>
				<a class='dep<?=$dep["dep_id"]?>' href='edit/<?=$subj["subj_id"]?>/'><?=$subj["subj_name"]?></a> [<a href='delete/<?=$subj["subj_id"]?>/' class='delet_subj'>Удалить</a>]
			</li>
<?php					} ?>
        </ul>
<?php				}
...
<?php       } ?>
        ...
        <form  method="post" name="add_subject" onsubmit="if(!subj_exists()) return false;">
            <h3>Название дисциплины:</h3>
            <input type="text" name="subject_name" id="subject_name" class="input_long" />
...
<?php 	} else {
        ...
        <form name="edit_subject" method="post">
            ...
            <h3>Кафедра, к которой относится дисциплина:</h3>
            <select name="department">
              <option value="0">Кафедра, к которой относится дисциплина:</option><?php $id=0;?>
<?php 					foreach($MODULE_OUTPUT["departments"] as $department) {
        if($department["faculty_id"]!=$id) {
					if($id) { ?>
					</optgroup>
...
<?php                        if($department["id"] == $subject["department_id"]) { ?>
...
<?php			}
		...
		<a  id="link14"onclick="show_choice(this, 'show_form_ed_zoch');">Заочная (Очно-заочная, Сокращённая, Индивидуально-ускоренная)</a> <br />
		</p></div>
...
<?php 	} ?>
	...
	<?php	if(isset($MODULE_OUTPUT['groups'])) { ?>
									<select name="group" size="1">
										<option></option>
	<?php 		foreach ($MODULE_DATA["output"]["groups"] as $gr) {
		if(($gr["form_education"]==2)||($gr["form_education"]==3)){
		?>
										<option value="<?=$gr["id"]?>"<?=(isset($display_variant) && $display_variant["group"] == $gr["id"] ? " selected='selected'" : "")?>><?=$gr["name"]?></option>
	<?php 		} } ?>
									</select>
