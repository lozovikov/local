...
<?php 						}
					...
					<table border="1">
						<tr>
							<th bgcolor="#BBBBBB">№ п.п</th>
							<th bgcolor="#BBBBBB">Год открытия</th>
							<th bgcolor="#BBBBBB">Уровень подготовки</th>
							<th bgcolor="#BBBBBB">Очная<br />форма<br />обучения</th>
							<th bgcolor="#BBBBBB">Заочная<br />форма<br />обучения</th>
							<th bgcolor="#BBBBBB">Ведется<br />набор</th>
							<th bgcolor="#BBBBBB">Продолжительность</th>
...
<?php 					foreach($MODULE_OUTPUT["speciality_edit"] as $edit) {
					    ...
					    <tr>
					        <td align="center"><?=$i?></td>
					        <td>
								<input type="hidden" name="spec_type[<?=$edit["type"]?>][st_type_id]" value="<?=$edit["id"]?>" />
								<input type="text" name="spec_type[<?=$edit["type"]?>][year_open]" value="<?=$edit["year_open"]?>" />
					        </td>
					        <td>
								<select name="spec_type[<?=$edit["type"]?>][type]" size="1">
<?php						switch ($edit["type"]) {
                                case secondary: ?>
									<option value="secondary" selected="selected">Среднее професcиональное образование</option>
...
<?php 							break;
					        ...
					        <td align="center"><input type="checkbox" name="spec_type[<?=$edit["type"]?>][internal_tuition]" <?php if ($edit["internal_tuition"]==1){?>checked <?php }?> /></td>
					        <td align="center"><input type="checkbox" name="spec_type[<?=$edit["type"]?>][correspondence_tuition]" <?php if ($edit["correspondence_tuition"]==1){?>checked <?php }?> /></td>
					        <td align="center"><input type="checkbox" name="spec_type[<?=$edit["type"]?>][has_vacancies]" <?php if ($edit["has_vacancies"]){?>checked <?php }?> /></td>
					        <td align="center"><input type="text" name="spec_type[<?=$edit["type"]?>][data_training]" value="<?=$edit["data_training"]?>" /></td>
							<td align="center"><input type="text" name="spec_type[<?=$edit["type"]?>][qualification]" value="<?=$edit["qualification"]?>" /></td>                       
					    </tr>
<?php 					} ?>
					</table><br />
...
<?php						if ($edit["type"] == "magistracy") {
								...
								foreach ($MODULE_OUTPUT["exams"] as $exam) { ?>
						<input type="checkbox" name="spec_type[<?=$edit["type"]?>][exams][<?=$exam["id"]?>]"<?=(($exam["need"] && $exam['type'] == $edit['type']) ? " checked='checked'" : "")?> /><?=$exam["name"]?><br />
<?php 							}
							} ?>
						<textarea name="spec_type[<?=$edit["type"]?>][description]" cols="60" rows="15" class="wysiwyg"><?=$edit["description"]?></textarea>
					</fieldset><br />
<?php 					} ?>
					<p>
						<input type="submit" value="Сохранить изменения" />
