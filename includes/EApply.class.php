...
/*		// Обработка файла параметров
		...
		$this->output["mode"] = $this->output["scripts_mode"] = $this->config["general_settings"]["output_mode"];
...
//										"image_height" => $MAIN_OUTPUT_HEIGHT,
    ...
    $DB->AddJoin("nsau_qualifications q.id", "st.qual_id", "nsau_spec_type"); 
		$DB->AddField("s.code"); 
    $DB->AddField("st.id");
		$DB->AddField("s.name"); 
    ...
    $DB->AddCondFS("st.has_vacancies", "=", 1);//echo $DB->SelectQuery(); 
		$res = $DB->Select();
	...
	function show_applies() {
		...
		$DB->SetTable("nsau_applies", "a");
    $DB->AddExp("a.*");
    $DB->AddJoin("nsau_spec_type st1.id", "a.speciality1", "nsau_applies");
    $DB->AddJoin("nsau_specialities s1.id", "st1.spec_id", "nsau_applies");
    $DB->AddJoin("nsau_qualifications q1.id", "st1.qual_id", "nsau_applies");
    $DB->AddField("st1.code", "code1"); 
    $DB->AddField("s1.name", "spec_name1");
    $DB->AddField("q1.name", "qual_name1");
    
    $DB->AddJoin("nsau_spec_type st2.id", "a.speciality2", "nsau_applies");
    $DB->AddJoin("nsau_specialities s2.id", "st2.spec_id", "nsau_applies");
    $DB->AddJoin("nsau_qualifications q2.id", "st2.qual_id", "nsau_applies");
    $DB->AddField("st2.code", "code2"); 
    $DB->AddField("s2.name", "spec_name2");
    $DB->AddField("q2.name", "qual_name2"); 
    
    $DB->AddJoin("nsau_spec_type st3.id", "a.speciality3", "nsau_applies");
    $DB->AddJoin("nsau_specialities s3.id", "st3.spec_id", "nsau_applies");
    $DB->AddJoin("nsau_qualifications q3.id", "st3.qual_id", "nsau_applies");
    $DB->AddField("st3.code", "code3"); 
    $DB->AddField("s3.name", "spec_name3");
    $DB->AddField("q3.name", "qual_name3");   
		$res = $DB->Select();
    ...
    $DB->AddField("s3.name", "spec_name3");
    $DB->AddField("q3.name", "qual_name3");
		$DB->AddCondFS("a.id", "=", $id);//echo $DB->SelectQuery();
		$res = $DB->Select();
		$row = $DB->FetchAssoc($res);
