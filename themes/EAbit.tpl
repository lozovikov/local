...
if(isset($MODULE_OUTPUT["edit_type"])) { 
	$type = $MODULE_OUTPUT["edit_type"]; 
	?>
	<h2>Редактировать направление</h2>
        ...
        <td rowspan="2" bgcolor="#f9dfc5" align="center">Квалификация</td>
				...
				<td bgcolor="#f9dfc5" colspan="2" align="center">Количество коммерческих мест</td>
				<td bgcolor="#f9dfc5" rowspan="2" align="center">Количество классов (для СПО)</td>
				<td rowspan="2" bgcolor="#f9dfc5" align="center">Есть заочные места</td>
			...
			<tr>
				...
				<td bgcolor="#f9dfc5" align="center">&nbsp;</td>
				<td bgcolor="#f9dfc5" align="center">Заочных</td>
			</tr>
			<tr>
					<td><input type="text" name="code" value="<?php echo $type->code; ?>" size="6" /></td>
					<td><input type="text" name="type" value="<?php echo $type->type; ?>" /></td> 
          <?php  ?><td><select name="qualification" style="width: 200px; ">
          <option value="0">Не указана</option>
          <?php foreach($MODULE_OUTPUT["qualifications"] as $qualification) { ?>
          <option value="<?php echo $qualification->id; ?>"<?php if($type->qual_id == $qualification->id) { ?> selected="selected"<?php } ?> data-old_type="<?php echo $qualification->old_type; ?>" title="<?php echo $qualification->old_type; ?>"><?php echo $qualification->name; ?></option>
          <?php } ?>
          </select></td><?php  ?>
					<td><input type="text" name="budget" size="6" value="<?php echo $type->budget; ?>" /><input type="hidden" name="id" value="" /></td>
					<td><input type="text" name="purpose" size="6" value="<?php echo $type->purpose; ?>" /></td>
					<td><input type="text" name="commerce" size="6" value="<?php echo $type->commerce; ?>" /></td>
					<td><input type="text" name="commerce_ext" size="6" value="<?php echo $type->commerce_ext; ?>" /></td>
					<td><select name="classes">
					<option value="0">Не указано</option>
					<option value="9"<?php if($type->classes == 9) { ?> selected="selected"<?php } ?>>9</option>
					<option value="11"<?php if($type->classes == 11) { ?> selected="selected"<?php } ?>>11</option>
					</select></td>
					<td><input type="hidden" name="has_ext" value="0" /><input type="checkbox" name="has_ext" value="1" <?php if($type->has_ext) { ?> checked="checked"<?php } ?>/></td>
					<td><input type="text" name="sortorder" size="2" value="<?php echo $type->sortorder; ?>" /></td>
					<?php /* ?><td>
...
<?php } elseif (isset($MODULE_OUTPUT['requests_info'])) {
	...
	<table style="border-collapse: separate;" border="1" cellspacing="1" cellpadding="1" width="620" align="center">
		<tbody>
			<tr>
				...
				<?php if ($MODULE_OUTPUT["request_mode"] != "vpo-ev" && $MODULE_OUTPUT["request_mode"] != "magistracy-ex"/*$requests_info["sum"]["budget"] != 0*/) { ?>
				<td bgcolor="#f9dfc5">
	...
	foreach ($requests_info as $faculty_name=>$faculty_info) {
		if (($MODULE_OUTPUT["request_mode"] != "spo" &&  $MODULE_OUTPUT["request_mode"] != "spo-ex")|| $faculty_name == "sum") { ?>
			<tr style="background-color: #ccff33;">
				...
				<td style="text-align: center;"><?php if ($MODULE_OUTPUT["request_mode"] != "vpo-ev" && $MODULE_OUTPUT["request_mode"] != "magistracy-ex"/*$requests_info["sum"]["budget"] != 0*/) { ?><?=($faculty_info['contest']?:'');?><?php } ?></td>
			</tr>
<?php 	}
		foreach ($faculty_info['specs'] as $spec_info) { ?>
			<tr style="text-align:center">
				<td>
					<div><a href="<?=$MODULE_OUTPUT['abit_list_base_href'].$spec_info->id?>"><?=$spec_info->code?></a></div>
				</td>
				...
				<td style="text-align: center;"><a href="<?=$MODULE_OUTPUT['abit_list_base_href'].$spec_info->id?>"><?=$spec_info->req_count?></a></td>
				<td style="text-align: center;"><?php if ($MODULE_OUTPUT["request_mode"] != "vpo-ev" && $MODULE_OUTPUT["request_mode"] != "magistracy-ex"/*$requests_info["sum"]["budget"] != 0*/) { ?><?=($spec_info->contest?:'')?><?php } ?></td>
			</tr>
<?php	}
	} ?>
		</tbody>
			...
			<table id='types_info' style="border-collapse: separate;" border="1" cellspacing="1" cellpadding="1" width="620" align="center">
				<tbody>
					<tr>
						...
						<td bgcolor="#f9dfc5">
							<p style="text-align: center;">Кол-во коммерческих мест (заочная форма)</p>
						</td>
						<td bgcolor="#f9dfc5">
	...
	foreach ($MODULE_OUTPUT['edit_info'] as $faculty_name=>$faculty_info) {
		$faculty_ind++; ?>
					<tr style="background-color: #ccff33;">
						...
						<td style="text-align: center; font-weight: bold;" id="<?=($faculty_name!='sum'?$faculty_ind:'sum').'-commerce_ext'?>"><?php if($faculty_info['has_ext']) { ?><?=$faculty_info['commerce_ext']?><?php } else { ?>&nbsp;<?php } ?></td>
...
<?php
		foreach ($faculty_info['specs'] as $spec_info) { ?>
					<tr style="text-align:center">
						...
						<td style="text-align: left;" height="20"><a href="/office/speciality_directory/edit_speciality/<?=$spec_info->spec_id; ?>/"><?=trim($spec_info->type)?></a>
						<br /><small><a href="?edittype=<?php echo $spec_info->id; ?>">Редактировать</a></small></td>
						...
						<td style="text-align: center;"><?php if($spec_info->has_ext/*  && $spec_info->spec_type == "magistracy" */) { ?><?=$MODULE_OUTPUT["auth"]?'<input class="info_cell" type="text" id="'.$faculty_ind.'-commerce_ext-'.$spec_info->id.'" style="text-align:center; width:40px; border: 1px solid;" value="'.$spec_info->commerce_ext.'"/>':$spec_info->commerce_ext?><?php } else { ?><?php echo '<input type="hidden" id="'.$faculty_ind.'-commerce_ext-'.$spec_info->id.'" style="text-align:center; width:40px; border: 1px solid;" value="'.$spec_info->commerce_ext.'"/>';?><?php } ?></td>
	...
	<h2>Добавить направление</h2>
        ...
        <td rowspan="2" bgcolor="#f9dfc5" align="center">Квалификация</td>
          ...
          <?php } ?>
					...
					<td><input type="text" name="commerce_ext" size="6" value="" /></td>
					<td><select name="classes">
					<option value="0">Не указано</option>
					<option value="9">9</option>
					<option value="11">11</option>
					</select></td>
					<td><input type="hidden" name="has_ext" value="0" /><input type="checkbox" name="has_ext" value="1" /></td>
...
<?php
  ...
  <script>/*jQuery(document).ready(function($) { AjaxAbitForm(<?=$MODULE_OUTPUT['module_id']?>, '<?=$MODULE_OUTPUT['abitprogressfile']?>'); });*/</script>
	...
	<form <? if (count($MODULE_DATA["output"]["messages"]["good"])) { ?>style="display: none;"<? } ?> action="/office/abit/" method="post" name="abitform" id="abitform" enctype="multipart/form-data">
		...
		<input name="type" value="vpo" type="radio" checked="checked" />Файл содержит данные абитурентов только ВО и магистратуры<br />
		<?php /* ?><input name="type" value="vpo_ev" type="radio"  />Файл содержит данные абитурентов только ВПО(вечернее)<br /><?php */ ?>
		<?php } ?>
...
<?php
	foreach($MODULE_OUTPUT["types"] as $type) {
		...
		<option value="<?=$EE["engine_uri"];?>?type=<?=$type["id"]?>"<?=$selected?>><?=$type["code"]?> - <?=$type["type"]?></option>
<?php
	} ?>
	</select>
<?php
	if(isset($_GET["type"])) { ?>
		...
		<p align="center">
			<?=(isset($MODULE_OUTPUT['isEvening']) && $MODULE_OUTPUT['isEvening']) ? '' : '<strong>Общее количество бюджетных мест: <span onclick="window.location = \'/entrant/request'.($MODULE_OUTPUT['isSpo'] ? '/request_SPO' . ($MODULE_OUTPUT['isExt'] ? '_ex' : '') : '').'/\';">'.$MODULE_OUTPUT["type"]["budget"].'</span>'?>
...
<?php 	} ?>
    ...
    <?php if($ent == 'Экзамены') { ?><h2>На базе среднего общего образования. 
    ...
    <?php if($ent == 'Собеседование') { ?>
	    <br /><h2>На базе профессионального образования (устный экзамен по направлению).</h2><?php } ?>
    <?php } ?>
...
<?php 	$category_prev = "";
		...
		foreach($MODULE_OUTPUT["people"][$ent] as $man) {
			...
			<tr><td colspan="<?php if(!$MODULE_OUTPUT["isMagistracy"] && !$MODULE_OUTPUT["isSpo"]  && $ent != "Собеседование") { ?>6<?php } else { ?>3<?php } ?>"><strong><?=$category?></strong></td></tr>
<?php 			} ?>
			<tr>
				<td><?=$i.".&nbsp;"?>
<?php 			if($ent != "Собеседование" && /*$man["acceptance"] == "Рекомендован" || $man["acceptance"] == "Зачислен")*/ ($i<=$MODULE_OUTPUT["type"]["budget"] || $man["dogovor"]=="заключен") && $man["old_type"]!="secondary" && $man["form"]!= "вечерняя") { ?>
					<strong>
<?php			} ?>
						<a href="/people/<?=$man["id"]?>/" <?=((/*$man["acceptance"] == "Рекомендован"*/0) ? " title='Рекомендован к зачислению'": (/*$man["dogovor"]=="заключен" ? ' title="Договор заключен" ' :*/ ''))?>>
...
<?php 			} ?>      
        ...
        <?php if(!$MODULE_OUTPUT["isMagistracy"] && !$MODULE_OUTPUT["isSpo"] && $ent != "Собеседование") { ?>
        <td><?php if($man["exam_rus"]) { ?><?=$MODULE_OUTPUT["isSpo"] ? sprintf("%.02f", number_format((float)(float)str_replace(",", ".", $man["exam_rus"]), 2, ",", "")) : $man["exam_rus"];?><?php } else { ?>0<?php } ?></td>
        <td><?php if($man["exam_prof"]) { ?><?=$MODULE_OUTPUT["isSpo"] ? sprintf("%.02f", (float)str_replace(",", ".", $man["exam_prof"])) : $man["exam_prof"]?><?php } else { ?>0<?php } ?></td>
        <td><?php if($man["exam_extra"]) { ?><?=$MODULE_OUTPUT["isSpo"] ? sprintf("%.02f", (float)str_replace(",", ".", $man["exam_extra"])) : $man["exam_extra"]?><?php } else { ?>0<?php } ?></td>
        <?php } ?>
				<td title="Общий балл"><strong><?=$MODULE_OUTPUT["isSpo"] ? /*sprintf("%.02f", (float)str_replace(",", ".", $man["ege"]))*/ number_format((float)str_replace(",", ".", $man["ege"]), 2, ",", "") : $man["ege"]; ?></strong></td>
				<td>
...
<?php			} elseif($man["acceptance"] == 5) { ?>
          <span style="font-weight: bold; color: #0c0;">Рекомендован к заключению договора</span>
          <?php } elseif (!CF::IsNaturalNumeric($man["acceptance"])) { ?>
          <span style="font-weight: bold; color: #0c0;"><?=$man["acceptance"];?></span>
          <?php } ?>
