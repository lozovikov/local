		...
		switch ($this->mode) {
			...
			case "section": {
				...
				if ($this->module_uri || !isset($parts[7])) {
					...
					$subsections_items_num = isset($parts[5]) ? (int)$parts[5] : false;
					$include_subsections_subitems = (isset($parts[6]) && $parts[6]);
					
					 $subsections_items_num=1;
				...
				switch ($POST_ACTION)
				{
					case "add_section":
						if (isset($POST_DATA["pid"], $POST_DATA["uri_part"], $POST_DATA["title"], $POST_DATA["descr"], $POST_DATA["text"]) && CF::IsNaturalOrZeroNumeric($POST_DATA["pid"]))
						{
							...
							if (!$this->status)
							{
								...
								$this->output["scripts_mode"] = "add_section";
					...
					case "save_section":
						if (isset($POST_DATA["id"], $POST_DATA["pid"], $POST_DATA["uri_part"], $POST_DATA["title"], $POST_DATA["descr"], $POST_DATA["text"]) && CF::IsNaturalOrZeroNumeric($POST_DATA["id"]) && CF::IsNaturalOrZeroNumeric($POST_DATA["pid"]))
						{
							...
							if (!$this->status)
							{
								$this->display_variant = $this->output["scripts_mode"] = "edit_section";
								$this->form_data = array(
									"id" =>  $POST_DATA["id"],
									"pid" => $POST_DATA["pid"],
									"pos" => $POST_DATA["pos"],
					...
					case "add_item":
						...
						if (isset($POST_DATA["section_id"], $POST_DATA["descr"], $POST_DATA["text"], $FILE_DATA) && CF::IsNaturalOrZeroNumeric($POST_DATA["section_id"]) && CF::IsNonEmptyStr($FILE_DATA["tmp_name"]) && $FILE_DATA["error"] == 0)
						...
						{
							$this->status = false;  
							if ($FILE_DATA["error"] == UPLOAD_ERR_FORM_SIZE || $FILE_DATA["error"] == UPLOAD_ERR_INI_SIZE)
							{
								$this->status = false;
								$this->output["messages"]["bad"][] = 305; // размер файла превышает максимально допустимый
							}
